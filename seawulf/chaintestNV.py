...
for seed in range(1,MAX_SEED+1):
    ...
    shapefile = gpd.read_file(r'NVPrecinctsTest.geojson')
    ...
    #     with open('./ensembleNV.json', 'a') as f:
    #         json.dump(ensembleData, f) 

if type(highestPopVariationDF) != type(None):
    # print(highestPopVariationDF)
    highestPopVariationDF.to_file("Nevada Highest PopVar of " + str(highestPopVariation)+ ".geojson", driver="GeoJSON")
    print("pop done")
if type(highestGeoVariationDF) != type(None):
    # print(highestGeoVariationDF)
    highestGeoVariationDF.to_file("Nevada Highest GeoVar of " + str(highestGeoVariation)+".geojson", driver="GeoJSON")
    print("geo done")
if type(mostDemocraticFavoredDF) != type(None):
    # print(mostDemocraticFavoredDF)
    mostDemocraticFavoredDF.to_file("Nevada Democratic Favored " + str(mostDemocraticFavored) + " seats.geojson", driver="GeoJSON")
    print("demo done")
if type(mostRepublicanFavoredDF) != type(None):
    # print(mostRepublicanFavoredDF)
    mostRepublicanFavoredDF.to_file("Nevada Republic Favored " + str(mostRepublicanFavored) + " seats.geojson", driver="GeoJSON")
    print("rep done")
...
with open('NVElecRep.txt', 'w') as f:
    f.writelines(elecDataRep)

with open('NVElecDem.txt', 'w') as f:
    f.writelines(elecDataDem)

with open("NVVariation.txt", 'w') as f:
    f.writelines(variations)